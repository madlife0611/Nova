<?php 
	trait HomeModel{
		public function modelFlashSale(){
			...
			$query = $db->query("select * from products where discount >= 25 limit 0,5");
			return $query->fetchAll();
		}
		//san pham noi bat (6 san pham)
		public function modelHotProduct(){
			$db = Connection::getInstance();
			$query = $db->query("select * from products where hot = 1 order by id asc limit 0,8");
			return $query->fetchAll();
		}
		...
		public function modelNewProduct(){
			$db = Connection::getInstance();
			$query = $db->query("select * from products order by id desc limit 0,3");
			return $query->fetchAll();
		}
		...
		//san 2 tin noi bat de hien thi o trang chu
		public function modelHotNews(){
			$db = Connection::getInstance();
			$query = $db->query("select * from news order by id desc limit 0,4");
			return $query->fetchAll();
		}
	}
 ?>